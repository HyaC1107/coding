...
export default function ConsPayAdvertisement2(): JSX.Element {
  ...
  const handlePayment = async () => {
    if (!planId) {
      Alert.alert('오류', '광고 플랜 정보가 없습니다.');
      return;
    }
    if (!check1 || !check2 || !check3 || !check4) {
      Alert.alert('알림', '모든 약관에 동의하고 서명해주세요.');
      return;
    }
    try {
      await post('/ad/payments', {planId});
      navigation.navigate('ConsPayAdvertisement3');
    } catch (error: any) {
      Alert.alert('오류', error.response?.data?.message || error.message || '결제 처리에 실패했습니다.');
    }
  };
