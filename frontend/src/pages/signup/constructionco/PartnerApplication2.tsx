...
export default function PartnerApplication2(): JSX.Element {
  ...
  const [form, setForm] = useState({
    ...
    SNS: '',
    youTube: '',
    guarantee: '', // 'none', 'possible', 'issued' etc.
  });
  ...
  const handleImagePick = () => {
    launchImageLibrary({mediaType: 'photo'}, response => {
      ...
      if (response.assets && response.assets.length > 0) {
        setCertificates([...certificates, response.assets[0]]);
      }
    });
  };

  const handleNext = () => {
    navigation.navigate('PartnerApplication3', {
      ...router.params,
      ...form,
      certificates: JSON.stringify(certificates.map(c => c.fileName)), // Temporary mock for array of strings if needed, but we'll send files in step 3
      certificateFiles: certificates, // Actual assets for step 3
    });
  };
  ...
  return (
    <SafeAreaView style={styles.rootWrapper}>
      <ScrollView
        ...
        <View style={[styles.greyBodyWrap, styles.marginBottom]}>
          ...
          <View style={{alignItems: 'center'}}>
            <Text style={[styles.uploadMent2]}>
              자격증을 {certificates.length}개 선택했습니다.
            </Text>
            <Text style={[styles.uploadMent2]}>
        ...
        <View style={[styles.greyBodyWrap, styles.marginBottom]}>
          <View style={styles.fileUploadWrap}>
            <TouchableWithoutFeedback onPress={() => setForm({...form, guarantee: 'issued'})}>
              <View style={[styles.fileUploadBox2, form.guarantee === 'issued' && {borderColor: '#4545FD'}]}>
                <View
                  style={{
                    height: '100%',
                  ...
                  <View style={{alignItems: 'center', gap: 2}}>
                    <Text style={[styles.uploadMent, form.guarantee === 'issued' && {color: '#4545FD'}]}>보증기관발급</Text>
                    <Text style={[styles.uploadMent, form.guarantee === 'issued' && {color: '#4545FD'}]}>유/무</Text>
                  </View>
                </View>
              </View>
            </TouchableWithoutFeedback>
            <TouchableWithoutFeedback onPress={() => setForm({...form, guarantee: 'possible'})}>
              <View style={[styles.fileUploadBox2, form.guarantee === 'possible' && {borderColor: '#4545FD'}]}>
                <View style={{height: '100%', justifyContent: 'center'}}>
                  <View style={{alignItems: 'center', gap: 2}}>
                    <Text style={[styles.uploadMent, form.guarantee === 'possible' && {color: '#4545FD'}]}>보증가능</Text>
                    <Text style={[styles.uploadMent, form.guarantee === 'possible' && {color: '#4545FD'}]}>여부</Text>
                  </View>
                </View>
              </View>
            </TouchableWithoutFeedback>
