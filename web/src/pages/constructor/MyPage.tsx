...
const ConstructorMyPage = () => {
  ...
  if (isLoading) return <ConstructorLayout><p>로딩 중...</p></ConstructorLayout>;
    ...
    <ConstructorLayout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '40px' }}>
        ...
        <form onSubmit={handleUpdate} style={{ backgroundColor: 'white', padding: '40px', borderRadius: 'var(--border-radius)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
          ...
          <Input label="대표자명" name="name" defaultValue={company?.userId?.name} />
          <Input label="이메일" name="email" defaultValue={company?.userId?.email} />
          <Input label="대표번호" name="phoneNumber" defaultValue={company?.userId?.phoneNumber} />
          <Input label="주소" name="address" defaultValue={company?.address} />
      ...
      <div style={{ marginTop: '60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
          ...
          <div style={supportCardStyle}>가게 꾸미기</div>
